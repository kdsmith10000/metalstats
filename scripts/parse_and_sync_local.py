"""
Parse local warehouse stocks XLS files from the project root directory
and update data.json and Neon database for February 4th, 2026 data.
"""
...
# Local file mappings - files are in the project root
LOCAL_FILES = {
...
def sync_to_database(data, database_url):
    ...
    try:
        ...
        print(f"[OK] Database sync complete ({synced_count} metals synced)")
        return True
...
def main():
    print("=" * 70)
    print("  Local Warehouse Stocks Parser - February 4th, 2026")
    print("=" * 70)
    print()
    ...
    if data_file.exists():
        try:
            ...
            print(f"[INFO] Existing data last updated: {existing_data.get('_metadata', {}).get('last_updated', 'unknown')}")
        except:
            pass
    
    # Parse all local files from the project root
    all_data = {}
    ...
    # Keep existing metals if not parsed (like Platinum_Palladium combined entry)
    for metal in existing_data:
        if metal not in all_data and metal != '_metadata':
            # Keep Platinum_Palladium if we now have separate Platinum and Palladium
            if metal == 'Platinum_Palladium' and 'Platinum' in all_data and 'Palladium' in all_data:
                continue
    ...
    if database_url:
        ...
        print("=" * 70)
        success = sync_to_database(all_data, database_url)
        if success:
            print("[OK] All data synced to Neon database!")
    else:
